...
func TestSubdivisionInsertSiteTwoPoint1(t *testing.T) {
	...
	// check to see if 0,100 -> 0,0 still exists, it should not
	if e := triangleEdge[1].FindONextDest(insertPoints[0]); e != nil {
		t.Errorf("edge, expected nil got %v", wkt.MustEncode(e.AsLine()))
		return
	}
...
func TestSubdivisionInsertSiteTwoPoint2(t *testing.T) {
	...
	// check to see if 0,100 -> 0,0 still exists, it should not
	if e := triangleEdge[1].FindONextDest(insertPoints[0]); e != nil {
		t.Errorf("edge still exists, expected nil got %v", wkt.MustEncode(e.AsLine()))
		return
	}
